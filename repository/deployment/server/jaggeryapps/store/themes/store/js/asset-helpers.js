...
(function () {
    ...
    renderAssetsScroll = function(data){
    	var temp = '{{#slice assets size="4"}}<div class="row-fluid">';
			...
			temp += '		{{/if}}';		
			temp += '	<img src="{{#if images_thumbnail}}{{tenantedUrl ""}}/storage/{{../../../../rxt.shortName}}/{{../../id}}/{{../../thumbnail}}{{/if}}">';
			temp += '	</div> </a>';
			...
			temp += '			<ul>';
			temp += '				<li>';
			temp += '					<h4>{{traduction "Version"}}</h4>';
			temp += '					<a class="asset-version" href="#">{{overview_version}}</a>';
			temp += '				</li>';
			temp += '				<li>';
			temp += '					<h4>{{traduction "Category"}}</h4>';
			temp += '					<a class="asset-category" href="#">{{cap ../type}}</a>';
			temp += '				</li>';
			temp += '				<li>';
			temp += '					<h4>{{traduction "Author"}}</h4>';
			temp += '					<a class="asset-author" href="#">{{overview_provider}}</a>';					
			temp += '				</li>';
			temp += '			</ul>';
			temp += '			{{#if ../indashboard}}';
			temp += '			<a href="#" class="btn disabled btn-added">{{traduction "Bookmarked"}}</a>';
			temp += '			{{else}}';
			temp += '				{{# if ../../../../sso}}';			
			temp += '				<a href="{{url "/login"}}" class="btn btn-primary asset-add-btn">{{traduction "Bookmark"}}</a>';
			temp += '				{{else}}';							
			temp += '					<a href="#" class="btn btn-primary asset-add-btn">{{traduction "Bookmark"}}</a>';
			temp += '				{{/if}}';
